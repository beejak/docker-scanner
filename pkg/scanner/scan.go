...
// ScanOptions configures a scan (image, optional Dockerfile, severity filter, offline, cache).
type ScanOptions struct {
	Image      string   // Image reference (e.g. alpine:latest)
...
// Scan runs the scanner engine (Trivy) on the given image and optional Dockerfile.
// Returns raw findings (image vulnerabilities + Dockerfile misconfigurations if --dockerfile set); caller should pass to enricher then report generator.
